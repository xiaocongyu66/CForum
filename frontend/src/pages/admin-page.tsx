...
import { ExtendedAdminSettings } from '@/components/extended-admin-settings';

type AdminTab = 'overview' | 'categories' | 'users' | 'advanced';
...
export function AdminPage() {
	...
	const [users, setUsers] = React.useState<Array<{ id: number; email: string; username: string; role: string; verified: number; created_at: string; avatar_url?: string | null }>>([]);
					...
					<>
						...
						{tab === 'overview' && (
							<div className="space-y-6">
								...
								<Card>
									<CardHeader><CardTitle>站点设置</CardTitle></CardHeader>
									<CardContent className="space-y-4">
										...
										<div className="grid gap-3 sm:grid-cols-2">
											{([
												['notify_on_user_delete', '删除账号时通知用户'],
												['notify_on_username_change', '修改用户名时通知用户'],
												['notify_on_avatar_change', '修改头像时通知用户'],
												['notify_on_manual_verify', '手动验证通过时通知用户'],
												...
												<label key={key} className="flex items-center gap-2 text-sm">
													<input
														...
														onChange={(e) => setSystemSettings((s) => ({ ...s, [key]: e.target.checked }))}
													/>
													{label}
												</label>
											))}
							...
							<Card>
								...
								<CardContent className="space-y-4">
									...
									<div className="space-y-2">
										{categories.map((c) => (
											<div key={c.id} className="flex items-center justify-between rounded-md border p-3 text-sm">
												...
												<div className="flex items-center gap-2">
													{editingCategoryId === c.id ? (
														<>
															<Button variant="outline" size="sm" disabled={loading} onClick={() => updateCategory(c.id)}>保存</Button>
															<Button variant="outline" size="sm" onClick={() => { setEditingCategoryId(null); setEditingCategoryName(''); }}>取消</Button>
														</>
													) : (
										...
										{categories.length === 0 && <div className="text-sm text-muted-foreground">暂无分类</div>}
									</div>
								</CardContent>
							</Card>
						)}
													...
													<tr key={u.id} className="border-t">
														...
														<td className="px-3 py-2">
															<div className="flex flex-wrap gap-2">
																<Button variant="outline" size="sm" className="border-sky-500 text-sky-700 hover:bg-sky-50 dark:text-sky-400" onClick={() => openEdit(u)}>编辑</Button>
																{!u.verified && (
																	<>
																		<Button variant="outline" size="sm" className="border-emerald-500 text-emerald-700 dark:text-emerald-400" onClick={() => manualVerify(u.id)}>验证</Button>
																		<Button variant="outline" size="sm" className="border-amber-500 text-amber-800 dark:text-amber-400" onClick={() => resendEmail(u.id)}>重发</Button>
																	</>
																)}
																{user?.id !== u.id && (
																	<Button variant="destructive" size="sm" onClick={() => deleteUser(u.id)}>删除</Button>
																)}
															</div>
														</td>
													</tr>
			...
			<Dialog open={editOpen} onOpenChange={setEditOpen}>
				<DialogContent>
					...
					<DialogFooter>
						...
						<Button onClick={saveEdit} disabled={loading}>{loading ? '保存中...' : '保存'}</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</PageShell>
