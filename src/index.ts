...
import { handleEnhancedRoutes, getSetting, submitForModeration } from './enhanced-routes';
...
// ← 在这里插入数学验证码校验
const captchaMode = (await env.cforum_db
  .prepare("SELECT value FROM settings WHERE key='captcha_type'")
  .first<{ value: string }>())?.value || 'turnstile';

if (captchaMode === 'math') {
  const { captcha_id, captcha_answer } = body;
  if (!captcha_id || captcha_answer === undefined)
    return jsonResponse({ error: '请完成验证码' }, 400);
  const record = await env.cforum_db
    .prepare('SELECT answer, used, expires_at FROM captcha_store WHERE id=?')
    .bind(captcha_id).first<{ answer: string; used: number; expires_at: number }>();
  if (!record || record.used || record.expires_at < Math.floor(Date.now() / 1000))
    return jsonResponse({ error: '验证码已过期' }, 400);
  if (record.answer !== String(captcha_answer).trim())
    return jsonResponse({ error: '验证码错误' }, 400);
  await env.cforum_db.prepare('UPDATE captcha_store SET used=1 WHERE id=?').bind(captcha_id).run();
}
				...
				const { success, meta } = await env.cforum_db.prepare(
					'INSERT INTO users (email, username, password, role, verified, verification_token, uid) VALUES (?, ?, ?, "user", 0, ?, ?)'
				).bind(email, username, passwordHash, verificationToken, newUid).run();
		...
		if (url.pathname === '/api/posts' && method === 'POST') {
			try {
				...
				// AI 审核（可选）
const postReviewEnabled = await env.cforum_db
  .prepare("SELECT value FROM settings WHERE key='post_review_enabled'")
  .first<{ value: string }>();
if (postReviewEnabled?.value === '1') {
  const newPost = await env.cforum_db
    .prepare('SELECT id FROM posts WHERE author_id=? ORDER BY id DESC LIMIT 1')
    .bind(userPayload.id).first<{ id: number }>();
  if (newPost) {
    const reviewStatus = await submitForModeration(
      env.cforum_db, env, 'post', newPost.id,
      `${safeTitle} ${content}`, userPayload.id
    );
    if (reviewStatus !== 'approved') {
      return jsonResponse({ success: true, review_status: reviewStatus,
        message: '内容正在审核中，审核通过后将公开显示' }, 202);
    }
  }
}
...
// POST /api/posts/:id/comments  ← 新增，放在 GET comments 路由之后
if (url.pathname.match(/^\/api\/posts\/\d+\/comments$/) && method === 'POST') {
  ...
  try {
    ...
    if (!content || isVisuallyEmpty(content))
      return jsonResponse({ error: '评论不能为空' }, 400);
    if (content.length > 1000)
      return jsonResponse({ error: '评论过长 (Max 1000)' }, 400);
    if (hasControlCharacters(content))
      return jsonResponse({ error: '内容含非法字符' }, 400);

    // 自动分配楼层号
    const lastFloor = await env.cforum_db
      .prepare('SELECT MAX(floor_number) AS n FROM comments WHERE post_id=?')
      .bind(postId).first<{ n: number }>();
    const floorNum = (lastFloor?.n || 0) + 1;

    const { success } = await env.cforum_db.prepare(
      'INSERT INTO comments (post_id, parent_id, author_id, content, floor_number) VALUES (?,?,?,?,?)'
    ).bind(postId, parent_id || null, userPayload.id, content.trim(), floorNum).run();

    await security.logAudit(userPayload.id, 'CREATE_COMMENT', 'comment', postId, {}, request);
    return jsonResponse({ success, floor_number: floorNum }, 201);
  } catch (e) { return handleError(e); }
}
		
	const enhanced = await handleEnhancedRoutes(
  request, env, url, method, jsonResponse, security
);
